...
class CurrentUserProfileViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func fetchProfile() {
        StorageManager.shared.downloadProfilePicture(for: user.userName) { [weak self] (image, url) in
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        DatabaseManager.shared.isValidRelationShip(for: user, type: .followers) { isFollower in
            self.isFollowing = isFollower
        }
        ...
        let viewModel = ProfileHeaderViewModel(avatarImageURL: self.user.profilePictureURL ,
                                               ...
                                               isFollowing: self.isCurrentUserProfile ? nil : self.isFollowing) //NOTE: BOOL ? NİL : FALSE DEMEK: TRUE İSe NİL DÖNDÜR YOKSA FALSE DÖÖNDÜR DEMEK. YANİ BOOL ?? TRUE'İN TAM TERSİ GİBİ.
...
extension ProfileViewController: ProfileHeaderCollectionReusableViewDelegate {
    ...
    func profileHeaderCollectionReusableView(_ header: ProfileHeaderCollectionReusableView, didTapPrimaryButtonWith viewModel: ProfileHeaderViewModel) {
        ...
        if self.user.userName == currentUsername {
            ...
            //Follow or unfollow current user profile that we are viewing
        }
    }
